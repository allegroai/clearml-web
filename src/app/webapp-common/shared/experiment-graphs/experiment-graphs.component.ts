import {ChangeDetectionStrategy, ChangeDetectorRef, Component, ElementRef, EventEmitter, Input, Output, QueryList, Renderer2, ViewChild, ViewChildren} from '@angular/core';
...
export class ExperimentGraphsComponent {
  ...
  constructor(private changeDetection: ChangeDetectorRef, private adminService: AdminService) {
  }
  ...
  @Input() set splitSize(splitSize: number) {
    ...
    this.observer = new IntersectionObserver(this.graphInView.bind(this, 'resize'), options);
    this.prepareRedraw();
  }
  ...
  @Input() set metrics(graphs) {
    ...
    Object.values(this.graphsData).forEach((graphs: any[]) => {
      graphs.forEach((graph: Plotly.Frame) => {
        if (getOr(0, 'layout.images.length', graph) > 0) {
          graph.layout.images.forEach((image: Plotly.Image) => image.source = this.adminService.signUrlIfNeeded(image.source));
        }
      });
    });
    ...
    this.observer = new IntersectionObserver(this.graphInView.bind(this, 'redraw'), options);
  ...
  graphInView(redrawMode, entries) {
    for (let i = 0; i < entries.length; i++) {
      ...
      if (!graphComponent.alreadyDrawn || graphComponent.shouldRefresh) {
        graphComponent.drawGraph(true, redrawMode === 'resize');
      }
    }
  }
